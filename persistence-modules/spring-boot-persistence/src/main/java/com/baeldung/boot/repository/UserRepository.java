...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT u FROM Person u WHERE u.status = ?1 and u.name = ?2")
    User findUserByStatusAndName(Integer status, String name);

    @Query("SELECT u FROM Person u WHERE u.status = :status and u.name = :name")
    ...
    @Modifying
    @Query("update Person u set u.status = :status where u.name = :name")
